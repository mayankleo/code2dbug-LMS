...
import { CreditCard, UploadCloud, CheckCircle, Copy, ArrowLeft, ShieldCheck, User, Building, Hash } from 'lucide-react';
      ...
      <div className="min-h-screen bg-black text-white font-sans flex flex-col items-center justify-center p-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 md:p-12 max-w-lg w-full text-center animate-fadeIn">
            ...
            <p className="text-zinc-400 mb-8">
                We have received your payment details. Our team will verify the transaction ID <strong>{paymentData.transactionId}</strong> within 24 hours.
            </p>
            <button 
        ...
        <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl h-fit">
                ...
                <p className="text-zinc-400 text-sm mb-8">
                    Please make a transfer of <span className="text-white font-bold">₹500</span> to the account below.
                </p>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-black/40 p-4 rounded-xl border border-zinc-800 flex justify-between items-center group cursor-pointer hover:border-blue-500/50 transition-colors" onClick={() => handleCopy(bankDetails.accountNumber)}>
                        <div>
                            <p className="text-xs text-zinc-500 uppercase font-bold">Account Number</p>
                            <p className="font-mono text-lg tracking-wide text-blue-400">{bankDetails.accountNumber}</p>
                        </div>
                        <Copy size={18} className="text-zinc-600 group-hover:text-blue-400" />
                    </div>
                    <div className="bg-black/40 p-4 rounded-xl border border-zinc-800 flex justify-between items-center group cursor-pointer hover:border-blue-500/50 transition-colors" onClick={() => handleCopy(bankDetails.ifscCode)}>
                        <div>
                            <p className="text-xs text-zinc-500 uppercase font-bold">IFSC Code</p>
                            <p className="font-mono text-lg tracking-wide text-white">{bankDetails.ifscCode}</p>
                        </div>
                        <Copy size={18} className="text-zinc-600 group-hover:text-blue-400" />
                    </div>
                </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl h-fit">
                <div className="flex items-center justify-center mb-8">
                    <div className="flex items-center gap-2 text-green-500">
                        <div className="w-8 h-8 rounded-full bg-green-600 text-white flex items-center justify-center font-bold"><CheckCircle size={16}/></div>
                ...
                <p className="text-zinc-400 text-sm mb-6">Enter the details of the account you paid from.</p>

                <form onSubmit={handleSubmit} className="space-y-5">
                    
                    {/* Account Holder Name */}
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Account Holder Name *</label>
                        <div className="relative">
                            <User className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" size={18} />
                            <input 
                                type="text" name="accountHolderName" required
                                value={paymentData.accountHolderName} onChange={handleChange}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        {/* IFSC Code */}
                        <div>
                            <label className="block text-xs font-bold text-zinc-500 uppercase mb-1">IFSC Code *</label>
                            <div className="relative">
                                <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" size={18} />
                                <input 
                                    type="text" name="ifscCode" required
                                    value={paymentData.ifscCode} onChange={handleChange}
                                    className="w-full bg-black border border-zinc-700 rounded-xl pl-12 pr-4 py-3 text-white focus:border-blue-500 focus:outline-none" 
                    ...
                    {/* Transaction ID */}
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Transaction ID / UTR *</label>
                        <input 
                            type="text" name="transactionId" required
                            value={paymentData.transactionId} onChange={handleChange}
                    ...
                    {/* Upload Screenshot */}
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Upload Screenshot (Optional)</label>
