...
const SubmittedAssignment = memo(({ assignment, githubLink, submissionStatus, task }) => (
...
SubmittedAssignment.displayName = 'SubmittedAssignment';
...
const DescriptionList = memo(({ items }) => {
  if (items.length === 0) return null;

  if (items.length === 1) {
    return <p className="text-base text-left">{items[0]}</p>;
  }

  return (
    <ol className="space-y-2 list-decimal list-inside text-start">
      {items.map((para, index) => (
...
const AssignmentCard = ({ task, courseId, moduleId, onComplete }) => {
  ...
  const descriptionItems = useMemo(() => {
    if (Array.isArray(assignment.description)) return assignment.description;
    return assignment.description ? [assignment.description] : [];
  ...
  }, [isLinkValid, submit, courseId, moduleId, task?.taskId, task?.id, githubLink, onComplete]);

  // Memoized button class
  const buttonClass = useMemo(() => {
    if (submitting) return 'bg-zinc-600 cursor-wait';
    if (githubLink && isLinkValid) return 'bg-blue-600 hover:bg-blue-700';
    return 'bg-zinc-600 cursor-not-allowed';
  }, [submitting, githubLink, isLinkValid]);

  // Display link for submitted state
  const displayLink = githubLink || task?.githubLink;
    ...
    <div className="max-w-2xl mx-auto bg-zinc-900 border border-zinc-700 rounded-2xl p-8">
      <div className="text-center">
        ...
        <button
          ...
          disabled={!githubLink || !isLinkValid || submitting}
          className={`w-full py-3 rounded-lg font-medium text-white transition-colors flex items-center justify-center gap-2 ${buttonClass}`}
        >
          {submitting ? (
            <>
