...
export const downloadFinalCertificate = async ({
  ...
  // Preload all images to ensure they are available
  ...
  // Preload all images
  await Promise.all([
    preloadImage(signatureImg),
    preloadImage(msmeLogoImg),
    preloadImage(certificateBgImg),
  ]);
  ...
  const certificates = [
    {
      ...
      html: generateInternshipCertificate(studentName, courseName, conductedFrom, duration, signatureImg, certificateBgImg),
      width: 1056,  // 11" at 96 DPI
      height: 816,  // 8.5" at 96 DPI
      pdfWidth: 279.4,
      pdfHeight: 215.9,
    ...
    },
    {
      ...
      html: generateLetterOfCompletion(studentName, courseName, duration, certificateId, signatureImg),
    ...
    },
    {
      ...
      html: generateSkillCertificate(studentName, courseName, duration, certificateId, signatureImg, msmeLogoImg),
      width: 1056,  // 11" at 96 DPI
      height: 816,  // 8.5" at 96 DPI
      pdfWidth: 279.4,
      pdfHeight: 215.9,
...
function generateInternshipCertificate(studentName, courseName, conductedFrom, duration, signatureImg, certificateBgImg) {
  return `
      ...
      <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 50px 60px; box-sizing: border-box; text-align: center;">
        ...
        <div style="margin-bottom: 10px;">
          <div style="display: inline-block; position: relative;">
            <span style="font-size: 44px; font-weight: bold; color: #2c3e50; font-family: Georgia, serif;">NS</span>
          </div>
          <p style="font-size: 10px; color: #2c3e50; font-weight: 600; letter-spacing: 3px; margin: 6px 0 0 0;">N.A.I.R.SOLUTIONS</p>
        </div>

        <!-- Main Title -->
        <h1 style="font-size: 56px; font-weight: bold; color: #2c3e50; letter-spacing: 6px; margin: 8px 0 0 0; font-family: Georgia, serif;">CERTIFICATE</h1>
        <h2 style="font-size: 16px; font-weight: 500; color: #2c3e50; letter-spacing: 6px; margin: 4px 0 18px 0;">OF ACHIEVEMENT</h2>

        <!-- Presented To -->
        <p style="font-size: 14px; font-weight: 600; color: #2c3e50; letter-spacing: 3px; margin: 0;">THIS CERTIFICATE IS PRESENTED TO</p>

        <!-- Student Name -->
        <h3 style="font-size: 44px; color: #c77d4a; margin: 14px 0; font-family: 'Brush Script MT', 'Segoe Script', cursive; font-weight: normal;">${studentName}</h3>

        <!-- From -->
        <p style="font-size: 14px; color: #2c3e50; margin: 4px 0;">From <span style="font-weight: bold;">CODE2DBUG</span></p>

        <!-- Description -->
        <p style="font-size: 13px; color: #2c3e50; line-height: 1.5; max-width: 550px; margin: 10px auto;">
        ...
        <!-- Conducted From -->
        <p style="font-size: 13px; font-weight: 600; color: #2c3e50; margin: 12px 0 20px 0;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-end; width: 100%; margin-top: auto; padding: 0 16px;">
          ...
          <div style="text-align: center; max-width: 180px; background: rgba(255,255,255,0.9); padding: 8px 16px; border-radius: 4px;">
            <img src="${signatureImg}" alt="Signature" style="height: 50px; margin-bottom: 4px; display: block; margin-left: auto; margin-right: auto;" crossorigin="anonymous" />
            <div style="border-top: 2px solid #2c3e50; padding-top: 6px;">
              <p style="font-size: 11px; font-weight: bold; color: #2c3e50; margin: 0; letter-spacing: 1px;">PRAVIN.R.NAIR</p>
              <p style="font-size: 9px; color: #2c3e50; margin: 2px 0 0 0;">Company CEO</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style="padding: 40px 48px; height: calc(100% - 8px); box-sizing: border-box; display: flex; flex-direction: column; line-height: 1.7;">
          ...
          <div style="flex: 1; font-size: 14px; color: #475569; text-align: justify; display: flex; flex-direction: column; gap: 18px;">
            <p style="margin: 0;">
              ...
              <span style="font-weight: 700; background-color: #fef3c7; padding: 2px 4px; margin: 0 2px; color: #0f172a; border-bottom: 2px solid #fef08a;">${studentName}</span>.
            ...
            <p style="margin: 0;">
              During their
              <span style="background-color: #fef3c7; padding: 2px 4px; margin: 0 2px; border-bottom: 2px solid #fef08a;">${duration}</span>
              internship with our organization, where they successfully completed the
              <span style="background-color: #fef3c7; padding: 2px 4px; margin: 0 2px; border-bottom: 2px solid #fef08a;">${courseName}</span>
              program,
              <span style="background-color: #fef3c7; padding: 2px 4px; margin: 0 2px; border-bottom: 2px solid #fef08a;">${studentName}</span>
          ...
          <div style="margin-top: 32px;">
            ...
            <div style="margin-bottom: 8px;">
              <img src="${signatureImg}" alt="Signature" style="height: 56px; margin-bottom: 8px; display: block;" crossorigin="anonymous" />
            </div>
            ...
            <div style="margin-top: 24px; padding-top: 24px; border-top: 1px solid #e2e8f0; display: flex; gap: 60px; font-size: 13px; color: #64748b;">
              <div>
              ...
              <div>
                <span style="display: block; font-weight: 600; color: #0891b2; text-transform: uppercase; font-size: 11px; letter-spacing: 0.08em; margin-bottom: 4px;">Verification ID</span>
      ...
      <div style="position: relative; width: 100%; height: 100%; background: white; border: 14px solid #ca8a04; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 24px 40px 20px 40px;">
          ...
          <div style="display: flex; flex-direction: column; align-items: center;">
            <img src="${msmeLogoImg}" alt="MSME Logo" style="height: 56px;" crossorigin="anonymous" />
          </div>
        ...
        <div style="padding: 16px 40px 24px 40px; text-align: center;">
          ...
          <!-- Footer Section -->
          <div style="margin-top: 32px; display: flex; align-items: flex-end; justify-content: space-between; gap: 20px;">
            ...
            <div style="text-align: left; font-size: 11px; color: #374151; max-width: 280px; line-height: 1.5;">
              ...
              <p style="margin: 3px 0;"><span style="font-weight: 600;">Website:</span> www.code2dbug.com</p>
            </div>

            <!-- Certificate ID - Bottom Center -->
            <div style="text-align: center; flex-shrink: 0;">
              <p style="font-size: 12px; font-weight: 600; margin: 0 0 6px 0;">Certificate ID:</p>
              <p style="font-size: 16px; font-weight: bold; margin: 0; color: #1e293b;">${certificateId || 'CERT' + Date.now().toString().slice(-8)}</p>
            </div>

            <!-- Signature - Bottom Right -->
            <div style="text-align: center; flex-shrink: 0;">
              <img src="${signatureImg}" alt="Signature" style="height: 56px; margin: 0 auto 6px; display: block;" crossorigin="anonymous" />
              <div style="border-top: 1px solid #1f2937; padding-top: 6px; min-width: 140px;">
                <p style="font-size: 12px; font-weight: bold; margin: 0; color: #1e293b;">PRAVIN.R.NAIR</p>
                <p style="font-size: 10px; color: #4b5563; margin: 2px 0 0 0;">Company CEO</p>
                <p style="font-size: 10px; color: #4b5563; margin: 0;">NAIR SOLUTIONS</p>
              </div>
            </div>
          </div>
        </div>
